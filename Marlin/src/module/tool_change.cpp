...
inline void fast_line_to_current(const AxisEnum fr_axis) {
  ...
  void mtc_init(void) {
    mtc_activate_solenoid();
  }

  void mtc_activate_solenoid(void) {
    OUT_WRITE(SOL0_PIN, HIGH);
  }

  void mtc_deactivate_solenoid(void) {
    OUT_WRITE(SOL0_PIN, LOW);
  }

  inline void marnetic_tool_change(const uint8_t tmp_extruder, bool no_move) {
    if (no_move) return;

    constexpr float z_raise = MAGNETIC_TOOLCHANGER_Z_HOP;
    const float parking_positions[] = MAGNETIC_TOOLCHANGER_X_POS;
...
void tool_change(const uint8_t tmp_extruder, const float fr_mm_s/*=0.0*/, bool no_move/*=false*/) {
    ...
    if (tmp_extruder != active_extruder) {
      ...
      #elif ENABLED(MAGNETIC_TOOLCHANGER) // Magnetic Switching ToolChanger
        marnetic_tool_change(tmp_extruder, no_move);
